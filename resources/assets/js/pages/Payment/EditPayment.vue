  ...
  <modal-layout class="white">
    <v-card :flat="true">
      ...
      >
        ...
        <v-spacer/>
        <v-toolbar-title class="text-xs-center white--text">Edit Payment ID# {{ id }}</v-toolbar-title>
        <v-spacer/>
        <v-toolbar-items>
          <v-btn
            :loading="form.busy" 
...
export default {
  ...
  methods: {
    ...
    fetchPayment() {
      let self = this;
      axios.get(route("api.payment.edit", { payment: self.id })).then(response => {
        console.log(response.data);
        this.form.user_id = response.data.data.customer.id;
        this.form.date_paid = response.data.data.date_paid;
        this.form.transaction_id = response.data.data.transaction_id;
    ...
    updatePayment() {
      ...
      self.form.busy = true;
      self.form
        .post(route("api.payment.update",{payment: self.id}), self.form)
        .then(response => {
          console.log(response.data);
